...
- 现状：`bt_mock_world` 已模拟；**真实适配未实现**——需转发
  `grasp_hexapod_servo_cpp/GripperCommand`（ID99，open 脉冲 683 / clamp 840；受限先 open 复位再试）。
  见 TODO-4。
...
| # | 方法 | 语义 / 返回协议 | 对应节点 | 现状 |
|---|---|---|---|---|
| C1 | `switch_mode(target_mode)` | 执行模式（服务 A1 的桥接），返回 `(state,message)`，state∈RUNNING/SUCCESS/FAILED；幂等 | `RunMode(mode)` | mock 有；真实待实现（TODO-1/6） |
| C2 | `sensor_health()` | 读 `/grasp_hexapod/sensor_health` + `/grasp_hexapod/encoder_state`，返回 `{name:{online,fresh,freq_hz,age_s,reason}}` | `IsSensorDataOk`/`WaitSensorsReady` | mock 有；真实待实现（TODO-6） |
| C3 | `is_landing_confirmed()` | 订阅 encoder_state：landed→True；not_landed→False；normal=false→None | `IsLandingConfirmed` | mock 有；真实待实现 |
| C4 | `rtk_covariance_ok()` | `/fix` 协方差对角最大 ≤ 阈值（默认 0.04 m²） | `WaitRtkPrecise` | mock 有；真实待实现 |
| C5 | `hold_motion(reason)` | 零速保持（RTK 协方差超限停走、等待恢复） | `WaitRtkPrecise` | mock 有；真实待实现 |
| C6 | `receive_task_command()` | 解析 `/lora/command`：`…RELEASE…`→"release"；`…RECOVER…`→"recover"；无→None；其他→非法 | `WaitTaskCommand` | mock 有；真实桥接待实现（TODO-6，数据源已由 lora_node 提供） |
| C7 | `wait_deployment(dt)` | ⑨ 等待 `CMD,HEX,DEPLOY,…`；True=已下放 | `WaitDeployment` | mock 有；真实待实现 |
| C8 | `wait_winch_hoisted(dt)` | ⑫/㉜ 等待 `CMD,HEX,HOIST_DONE,…`；True=已回收 | `WaitWinchHoisted` | mock 有；真实待实现 |
| C9 | `report_status(status, dt)` | 发布 `/lora/status` `STA,HEX,<status>,<x>,<y>` | `ReportStatus` | mock 有；真实桥接待实现（TODO-6，数据源已由 lora_node 提供） |
| C10 | `read_remote_cmd()` | 读 `/grasp_hexapod/remote_cmd`，返回 dict（**仅遥控测试链**） | `CheckRemoteCmd` | mock 有；真实可用 remote_control |
...
- [ ] **TODO-4 GripperAct 真实适配**：`~/gripper_act` 转发 `grasp_hexapod_servo_cpp/GripperCommand`
  （ID99，open 683 / clamp 840；受限先 open 复位；带到位校验）。
