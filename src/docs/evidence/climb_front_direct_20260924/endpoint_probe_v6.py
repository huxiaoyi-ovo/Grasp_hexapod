...
OUT = Path(__file__).with_name('endpoint_probe_v6.json')
...
def main():
    ...
    if report['stopped_on_first_failure'] is None:
        ...
        probe('LB_TO_013', [.24, -.067694497636, .21, 0., 0., YAW], target, (0,),
              'LB=.13 m, body x=.24,z=.21; hold height across M support')
    if report['stopped_on_first_failure'] is None:
        target = anchors.copy(); target[3, 0] = .13
        target[3, 2] = cached_z(config, 3, .13, target[3, 1])
        probe('RB_TO_013', [.253, -.067694497636, .21, 0., 0., YAW], target, (3,),
              'RB=.13 m, body x=.253,z=.21; hold height across M support')
    if report['stopped_on_first_failure'] is None:
