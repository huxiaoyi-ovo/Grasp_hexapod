...
from build_climb_front import (  # noqa: E402
    MESH, TRANSLATION, P0, base_points, foot_sphere_signed_gap, load_triangles,
    solve_ik, support_margin, surface_height, world_from_base,
)
...
OUT = Path(__file__).with_name('endpoint_probe_v8.json')
...
IK_MAX_M = .004
YAW = -np.pi / 2.0
STL_LOCAL = load_triangles(MESH)
STL_WORLD = STL_LOCAL + TRANSLATION
LANDING_CHECKS = []
...
def ground_z(leg, x, y):
    z = GROUND_Z
    LANDING_CHECKS.append({
        'leg': LEGS[leg], 'xy_m': [float(x), float(y)], 'z_m': z,
        'kind': 'ground', 'source': 'explicit_ground_foot_center_z',
        'pass': abs(z - GROUND_Z) <= 1e-12,
    })
    return z


def endpoint_surface_z(config, leg, x, y, source_stage):
    stage = next(s for s in config['stages'] if s['name'] == source_stage)
    point = stage['anchor_knots'][-1][leg]
    if abs(point[0] - x) > 1e-8 or abs(point[1] - y) > 1e-8:
        raise ValueError(f'{source_stage} endpoint does not land {LEGS[leg]} at requested XY')
    z = float(point[2])
    expected = float(surface_height(STL_LOCAL, x, y))
    gap = float(foot_sphere_signed_gap(STL_WORLD, np.array((x, y, z), dtype=float)))
    passed = (-.0005 <= gap <= .002) and abs(z - expected) <= .002
    LANDING_CHECKS.append({
        'leg': LEGS[leg], 'xy_m': [float(x), float(y)], 'z_m': z,
        'kind': 'xiaolan', 'source': f'{source_stage}.anchor_knots[-1]',
        'surface_height_center_m': expected, 'foot_sphere_signed_gap_m': gap,
        'pass': passed,
    })
    if not passed:
        raise ValueError(f'{LEGS[leg]} landing fails STL sphere gap: {gap:.6f}m')
    return z


def validate_terminal_surface_anchors(anchors, stage_name):
    for leg in range(6):
        x, y, z = map(float, anchors[leg])
        expected = float(surface_height(STL_LOCAL, x, y))
        gap = float(foot_sphere_signed_gap(STL_WORLD, anchors[leg]))
        passed = (-.0005 <= gap <= .002) and abs(z - expected) <= .002
        LANDING_CHECKS.append({
            'leg': LEGS[leg], 'xy_m': [x, y], 'z_m': z,
            'kind': 'xiaolan', 'source': f'{stage_name}.anchor_knots[-1]',
            'surface_height_center_m': expected, 'foot_sphere_signed_gap_m': gap,
            'pass': passed,
        })
        if not passed:
            raise ValueError(f'{LEGS[leg]} terminal STL sphere gap fails: {gap:.6f}m')
...
def main():
    ...
    report = {
        ...
        'stages': [],
        'landing_height_stl_checks': LANDING_CHECKS,
    ...
    if report['stopped_on_first_failure'] is None:
        ...
        for leg in FRONT:
            target[leg, 0] = .34
            target[leg, 2] = endpoint_surface_z(config, leg, .34, target[leg, 1], 'G_MID_BACK_SYNC')
    ...
    if report['stopped_on_first_failure'] is None:
        ...
        for leg in MID:
            target[leg, 0] = .19
            target[leg, 2] = endpoint_surface_z(config, leg, .19, target[leg, 1], 'Q_MID_LOW')
    ...
    if report['stopped_on_first_failure'] is None:
        ...
        for leg in FRONT:
            target[leg, 0] = .40
            target[leg, 2] = endpoint_surface_z(config, leg, .40, target[leg, 1], 'R_FRONT_BODY_LIFT')
    ...
    if report['stopped_on_first_failure'] is None:
        ...
        target[0, 2] = ground_z(0, .055, target[0, 1])
    ...
    if report['stopped_on_first_failure'] is None:
        ...
        for leg in FRONT:
            target[leg, 0] = .48
            target[leg, 2] = endpoint_surface_z(config, leg, .48, target[leg, 1], 'Z_FRONT_48_SYNC')
        probe('FRONT_TO_048', [.170, -.067694497636, .19, 0., 0., YAW], target, FRONT,
              'F=.48 m, body x=.170,z=.19; reduced body advance to retain support COM margin')
    if report['stopped_on_first_failure'] is None:
        target = anchors.copy(); target[3, 0] = .055
        target[3, 2] = ground_z(3, .055, target[3, 1])
        probe('RB_TO_0055', [.18, -.067694497636, .195, 0., 0., YAW], target, (3,),
              'RB=.055 m, body x=.18,z=.195')
    if report['stopped_on_first_failure'] is None:
        probe('SIX_SUPPORT_BODY_0200', [.20, -.067694497636, .195, 0., 0., YAW], anchors, (),
              'M remains at .19; all feet fixed while body advances to x=.20,z=.195')
    if report['stopped_on_first_failure'] is None:
        target = anchors.copy(); target[0, 0] = .13
        target[0, 2] = ground_z(0, .13, target[0, 1])
        probe('LB_TO_013', [.215, -.067694497636, .195, 0., 0., YAW], target, (0,),
              'LB=.13 m, body x=.215,z=.195')
    if report['stopped_on_first_failure'] is None:
        target = anchors.copy(); target[3, 0] = .13
        target[3, 2] = ground_z(3, .13, target[3, 1])
        probe('RB_TO_013', [.23, -.067694497636, .195, 0., 0., YAW], target, (3,),
              'RB=.13 m, body x=.23,z=.195')
    if report['stopped_on_first_failure'] is None:
        probe('SIX_SUPPORT_BODY_0277', [.277, -.067694497636, .195, 0., 0., YAW], anchors, (),
              'all feet fixed; body x=.23 to .277 m, M remains at .19')
    if report['stopped_on_first_failure'] is None:
        target = anchors.copy()
        for leg in MID:
            target[leg, 0] = .235
            target[leg, 2] = endpoint_surface_z(config, leg, .235, target[leg, 1], 'AG2_MID_235_SYNC')
        probe('MID_TO_0235', pose, target, MID,
              'M=.235 m after body advance to x=.277,z=.195')
    if report['stopped_on_first_failure'] is None:
        terminal = by_name['AH_BACK_LOW_FIRST_SEGMENT']
        target = np.asarray(terminal['anchor_knots'][-1], dtype=float)
        validate_terminal_surface_anchors(target, 'AH_BACK_LOW_FIRST_SEGMENT')
