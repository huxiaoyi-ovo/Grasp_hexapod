// Independent ring chain, adapted from real_control_node; original node unchanged.
...
const char* RingControlNode::machineStateName(MachineState state) {
  switch (state) {
...
std::pair<bool, bool> RingControlNode::feedbackFrameState(
...
std::string RingControlNode::feedbackIssue(
    const JointAngles& q_cur, const std::array<double, 6>& feedback_stamp,
...
RingControlNode::NavigationInput::NavigationInput(ros::NodeHandle& pnh) {
...
bool RingControlNode::NavigationInput::validFrameImpl(
...
void RingControlNode::NavigationInput::baseCallback(
...
void RingControlNode::NavigationInput::xiaolanCallback(
    const geometry_msgs::PoseStamped::ConstPtr& message) {
  if (!validFrame(*message)) {
    return;
  }
...
void RingControlNode::NavigationInput::boundaryCallback(
...
void RingControlNode::NavigationInput::landingCallback(
    const std_msgs::Bool::ConstPtr& message) {
  std::lock_guard<std::mutex> guard(lock_);
  landing_confirmed_ = message->data;
}

NavigationState RingControlNode::NavigationInput::snapshot() const {
...
NavigationState RingControlNode::NavigationInput::motionSnapshot() const {
  const double now = nowSeconds();
  std::lock_guard<std::mutex> guard(lock_);
  NavigationState state;
  state.stamp = std::min(base_stamp_, xiaolan_stamp_);
...
RingControlNode::ImuInput::ImuInput(ros::NodeHandle& pnh) {
...
void RingControlNode::ImuInput::callback(const sensor_msgs::Imu::ConstPtr& message) {
  const double stamp = message->header.stamp.toSec();
...
RingControlNode::ImuInput::Snapshot RingControlNode::ImuInput::snapshot() const {
  const double now = nowSeconds();
  std::lock_guard<std::mutex> guard(lock_);
  Snapshot snap;
...
RingControlNode::BoolInput::BoolInput(ros::NodeHandle& nh,
...
void RingControlNode::BoolInput::callback(const std_msgs::Bool::ConstPtr& message) {
  std::lock_guard<std::mutex> guard(lock_);
  value_ = message->data;
  received_at_ = nowSeconds();
}

std::optional<bool> RingControlNode::BoolInput::snapshot() const {
  std::lock_guard<std::mutex> guard(lock_);
  return value_;
}

std::pair<std::optional<bool>, double> RingControlNode::BoolInput::snapshotWithTime()
    const {
  std::lock_guard<std::mutex> guard(lock_);
...
void RingControlNode::loadParameters(ros::NodeHandle& pnh) {
...
void RingControlNode::configureNavigationApproach(ros::NodeHandle& pnh) {
...
void RingControlNode::createInterfaces(ros::NodeHandle& nh, ros::NodeHandle& pnh) {
  ...
  switch_mode_server_ = nh.advertiseService(
      "/grasp_hexapod/switch_mode", &RingControlNode::switchModeService, this);
...
RingControlNode::RingControlNode(ros::NodeHandle& nh, ros::NodeHandle& pnh) {
...
void RingControlNode::joyCallback(const sensor_msgs::Joy::ConstPtr& message) {
...
double RingControlNode::readAxis(const Eigen::VectorXd& values, int index) {
...
double RingControlNode::bodyAxis(const Eigen::VectorXd& axes) const {
  // 把 RT/LT 两个同基准扳机合成为 [-1,1] 升降指令。
  return 0.5 * axis_body_scale_ *
         (readAxis(axes, axis_body_up_) - readAxis(axes, axis_body_down_));
}

Eigen::Vector4d RingControlNode::makeCommand(const Eigen::VectorXd& axes) const {
...
bool RingControlNode::manualCommandActive(const Eigen::VectorXd& axes) const {
  // 升降、平移或转向任一有效即接管导航。
...
void RingControlNode::remoteCmdCallback(
...
void RingControlNode::btHoldCallback(const std_msgs::String::ConstPtr&) {
...
std::shared_ptr<RingControlNode::BtRequest> RingControlNode::btRequestSnapshot()
    const {
  std::lock_guard<std::mutex> guard(bt_mutex_);
  if (bt_request_ == nullptr || bt_request_->final_result.has_value()) {
    return nullptr;
...
void RingControlNode::finishBtRequest(const std::shared_ptr<BtRequest>& request,
...
void RingControlNode::abortBtRequest(const std::string& reason) {
...
bool RingControlNode::switchModeService(
...
std::pair<bool, std::string> RingControlNode::actuateGripper(
...
void RingControlNode::runManualGripper(const std::string& action) {
...
void RingControlNode::maybeTriggerManualGripper(double dpad) {
...
void RingControlNode::startRealClimb(const JointAngles& q_cur,
                                     bool controls_ready) {
  ...
  if (!controls_ready || state_ != MachineState::Hold ||
      controller_->mode != RingController::kApproach) {
...
void RingControlNode::ensureDockMode() {
  // 按显式实机请求创建 DockMode，不给普通控制器增加 ROS 资源。
  if (dock_mode_instance_ != nullptr) {
    return;
  }
  ros::NodeHandle pnh("~");
  dock_perception_ = std::make_unique<RingPerceptionRos>(pnh, dock_system_config_);
  dock_mode_instance_ = std::make_unique<DockMode>(
      controller_->dt,
      [this](const JointAngles& q, bool sync) {
        return sync ? controller_->syncActualFeet(q) : controller_->kinematic.forwardBase(q);
      },
      [this]() { return dock_perception_->latest(); },
      [this]() { dock_perception_->reset(); },
      ...
      pnh.param("dock_leg_lift_height_m", 0.04),
      ...
      pnh.param("dock_alignment_speed_m_s", 0.02));
  controller_->attachDockMode(dock_mode_instance_.get());
}

void RingControlNode::startRealDock(const JointAngles& q_cur,
                                    bool controls_ready) {
  // 在实机输入新鲜时进入 DockMode 并保持当前反馈姿态。
  ...
  state_ = MachineState::Running;
  ROS_INFO("Y accepted: holding current posture, then raising body before ring docking");
}

std::optional<bool> RingControlNode::dockLockConfirmed() {
  // 只接受本次 Y 后、未过期的锁紧确认。
...
void RingControlNode::holdMotion(const std::string& reason, bool log) {
  // 停止推进步态并保持上力；再次按 A 才能恢复。
  if (state_ != MachineState::Running) {
    return;
  }
  if (controller_->mode == RingController::kDock) {
    if (dock_mode_instance_ != nullptr) {
...
void RingControlNode::processButtons(const Eigen::VectorXi& button_presses,
                                     bool controls_ready,
  ...
  if (controller_->mode == RingController::kClimb) {
...
std::pair<bool, std::string> RingControlNode::realClimbObservation() {
...
void RingControlNode::monitorRealClimb() {
  // 持续观察已启用的可选相对运动监控。
  if (!real_climb_monitor_active_ ||
      controller_->mode != RingController::kClimb) {
    return;
  }
  const auto [okay, reason] = realClimbObservation();
  if (okay) {
...
void RingControlNode::resetRealClimbSpeedDiagnostic() {
  climb_speed_diagnostic_.reset();
}

void RingControlNode::warnHardwareClimbPhaseHold() {
...
void RingControlNode::infoHardwareClimbActiveTrace() {
...
void RingControlNode::flushRealClimbSpeedDiagnostic(const std::string& reason) {
  // 输出并丢弃已完成阶段的聚合，不影响运动。
...
void RingControlNode::recordRealClimbSpeedDiagnostic(
...
void RingControlNode::startBtRequest(BtRequest& request, const JointAngles& q_cur) {
  ...
  if (mode == "walk") {
    ...
    controller_->setMode(RingController::kApproach);
...
void RingControlNode::finishBtModeIfTerminal(
...
bool RingControlNode::btHoldIsActive(double now) {
  const std::shared_ptr<BtRequest> request = btRequestSnapshot();
...
void RingControlNode::feedbackCallback(int leg_index, const std::string& leg_name,
...
std::optional<JointAngles> RingControlNode::updateControl(
  ...
  if (controller_->mode == RingController::kDock && dock_mode_instance_ &&
      dock_mode_instance_->state == DockMode::kWaitingTag) {
    ROS_WARN_THROTTLE(2.0, "%s", dock_mode_instance_->reason.c_str());
  }
...
void RingControlNode::publishTargets(const JointAngles& q_des) {
