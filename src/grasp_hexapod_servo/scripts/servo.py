...
诊断：
    ~enable_diagnostics为true时输出时序诊断；
    ~enable_voltage_read为true时读取并汇总供电电压（默认关闭，读取占用串口总线）。
...
class ServoSideNode:
    ...
    def __init__(self):
        ...
        # 电压读取独立开关：轮询电压占用串口总线，默认关闭。
        self.enable_voltage_read = bool(
            rospy.get_param("~enable_voltage_read", False)
        )
